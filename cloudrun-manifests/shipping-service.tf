resource "google_cloud_run_service" "shippingservice" {
  ...
  template {
    spec {
      ...
      containers {
        image = "gcr.io/google-samples/microservices-demo/shippingservice:v0.3.2"
        ports {
          container_port = local.ports["shippingservice"]
        }
        env {
