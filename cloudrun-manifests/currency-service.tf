resource "google_cloud_run_service" "currencyservice" {
  ...
  template {
    spec {
      ...
      containers {
        image = "gcr.io/google-samples/microservices-demo/currencyservice:v0.3.2"
        ports {
          container_port = local.ports["currencyservice"]
        }
        env {
