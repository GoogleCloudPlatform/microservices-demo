resource "google_cloud_run_service" "productcatalogservice" {
  ...
  template {
    spec {
      ...
      containers {
        image = "gcr.io/google-samples/microservices-demo/productcatalogservice:v0.3.2"
        ports {
          container_port = local.ports["productcatalogservice"]
        }
        env {
