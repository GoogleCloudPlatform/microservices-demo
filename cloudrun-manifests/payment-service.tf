resource "google_cloud_run_service" "paymentservice" {
  name     = "paymentservice"
  location = var.region
  ...
  template {
    spec {
      ...
      containers {
        image = "gcr.io/google-samples/microservices-demo/emailservice:v0.3.2"
        ports {
          container_port = local.ports["paymentservice"]
        }
        env {
