...
# replace release/ contents 
rm -rf "${REPO_ROOT}/release"
mkdir "${REPO_ROOT}/release"
cp -a "${REPO_ROOT}/dev-release/." "${REPO_ROOT}/release/"

# update version in manifests
CURRENT_VERSION=$(grep -A 1 VERSION ${REPO_ROOT}/release/*.yaml | grep value | head -n 1 | awk '{print $3}' |  tr -d '"')
find "${REPO_ROOT}/release" -name '*.yaml' -exec sed -i -e "s/${CURRENT_VERSION}/${NEW_VERSION}/g" {} \;
# tag image with explicit version
find "${REPO_ROOT}/release" -name '*.yaml' -exec sed -i -e "s/:latest/:${NEW_VERSION}/g" {} \;

# push release PR
git checkout -b "release/${NEW_VERSION}"
git add "${REPO_ROOT}/release/*.yaml"
