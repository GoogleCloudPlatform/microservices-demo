...
func (client *ThriftClient) PlaceOrder(ctx context.Context, req *pb.PlaceOrderRequest) (*pb.PlaceOrderResponse, error) {
	...
	prep, err := client.prepareOrderItemsAndShippingQuoteFromCart(ctx, req.UserId, req.UserCurrency, req.Address)
	if err != nil {
	...
	txID, err := client.chargeCard(ctx, &total, req.CreditCard)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "failed to charge card: %+v", err)
	}
	log.Infof("payment went through (transaction_id: %s)", txID)

	shippingTrackingID, err := client.shipOrder(ctx, req.Address, prep.cartItems)
	if err != nil {
		return nil, status.Errorf(codes.Unavailable, "shipping error: %+v", err)
	}

	_ = client.emptyUserCart(ctx, req.UserId)
	...
	if err := client.sendOrderConfirmation(ctx, req.Email, orderResult); err != nil {
...
func (cs *ThriftClient) prepareOrderItemsAndShippingQuoteFromCart(ctx context.Context, userID, userCurrency string, address *pb.Address) (orderPrep, error) {
	var out orderPrep
...
func (cs *ThriftClient) quoteShipping(ctx context.Context, address *pb.Address, items []*pb.CartItem) (*pb.Money, error) {
...
func (cs *ThriftClient) getUserCart(ctx context.Context, userID string) ([]*pb.CartItem, error) {
...
func (cs *ThriftClient) emptyUserCart(ctx context.Context, userID string) error {
	conn, err := grpc.DialContext(ctx, cs.cartSvcAddr, grpc.WithInsecure(), grpc.WithStatsHandler(&ocgrpc.ClientHandler{}))
	if err != nil {
...
func (cs *ThriftClient) prepOrderItems(ctx context.Context, items []*pb.CartItem, userCurrency string) ([]*pb.OrderItem, error) {
...
func (cs *ThriftClient) convertCurrency(ctx context.Context, from *pb.Money, toCurrency string) (*pb.Money, error) {
...
func (cs *ThriftClient) chargeCard(ctx context.Context, amount *pb.Money, paymentInfo *pb.CreditCardInfo) (string, error) {
...
func (cs *ThriftClient) sendOrderConfirmation(ctx context.Context, email string, order *pb.OrderResult) error {
...
func (cs *ThriftClient) shipOrder(ctx context.Context, address *pb.Address, items []*pb.CartItem) (string, error) {
