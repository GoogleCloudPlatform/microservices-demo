...
func printStatus(addr string, opt *Option) {
	...
	log.Infof("protocol: [%s], httpTransport: [%t], urlPath [%s] , secure: [%t], buffered: [%t], framed: [%t]",
		opt.Protocol, opt.HttpTransport, opt.HttpUrl, opt.Secure, opt.Buffered, opt.Framed)
}
