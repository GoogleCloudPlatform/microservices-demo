...
resource "google_compute_forwarding_rule" "inventory_psc_endpoint" {
  ...
  subnetwork            = data.google_compute_subnetwork.default_subnet.id  # Let GCP assign IP from subnet
  
  # This will get an IP from the default subnet to reach the inventory service
}
